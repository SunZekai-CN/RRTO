...
WORKDIR /opt/pytorch
RUN git checkout 8a1a93a9235a0774da586a925ed623a8dfe12288 && git submodule update --init --recursive
...
RUN curl -fsSL -v -o ~/miniconda.sh -O  https://repo.anaconda.com/miniconda/Miniconda3-py37_23.1.0-1-Linux-aarch64.sh && \
...
RUN PYTORCH_VERSION=$(pip list | grep torch | head -1 | awk '{print $2}') NVCC_FLAGS="-Xfatbin -compress-all -cudart shared" python3 setup.py install
