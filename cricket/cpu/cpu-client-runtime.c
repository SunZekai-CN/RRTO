...
cudaError_t cudaGetDevice(int* device)
{
     ...
     if (cached_device == -1)
    {
        ...
        cached_device = result.int_result_u.data;
        return result.err;
    }
    else
    {
        *device = cached_device;
        return 0; // result.err is 0
    }
}

int cached_devicecount = -1;
cudaError_t cudaGetDeviceCount(int* count)
{
#ifdef WITH_API_CNT
    api_call_cnt++;
#endif //WITH_API_CNT
    if(cached_devicecount == -1)
    {
        int_result result;
        enum clnt_stat retval_1;
        ...
        cached_devicecount = result.int_result_u.data;
        return result.err;
    }
    else
    {
        *count = cached_devicecount;
        return 0; // result.err is 0
    }
...
cudaError_t cudaGetLastError(void)
{
    ...
    if (cached_lasterror == -1)
    {
        ...
        cached_lasterror = result;
    }
    else
    {
        result = cached_lasterror;
    }
    return result;
}
